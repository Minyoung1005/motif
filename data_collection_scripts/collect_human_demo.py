...
fps_depth_downsample_factor = 6 # The depth frame rate will be fps_color/fps_depth_downsample_factor.
...
if "topdown" in args.view:
    ...
    config_1 = rs.config()
    config_1.enable_device('DEVICE_ID_topdown')
...
if "side" in args.view:
    ...
    config_2 = rs.config()
    config_2.enable_device('DEVICE_ID_sideview')
...
if not os.path.exists(f"./data/human/{date_str}"):
    os.makedirs(f"./data/human/{date_str}")
...
if save_video_color:
    if pipeline_1 is not None:
        writer_color_1 = imageio.get_writer(f"./data/human/{date_str}/traj{episode_count}_color.mp4", fps=fps_color)
    if pipeline_2 is not None:
        writer_color_2 = imageio.get_writer(f"./data/human/{date_str}/traj{episode_count}_sideview_color.mp4", fps=fps_color)
if save_video_depth:
    if pipeline_1 is not None:
        writer_depth_1 = imageio.get_writer(f"./data/human/{date_str}/traj{episode_count}_depth.mp4", fps=fps_color/fps_depth_downsample_factor)
    if pipeline_2 is not None:
        writer_depth_2 = imageio.get_writer(f"./data/human/{date_str}/traj{episode_count}_sideview_depth.mp4", fps=fps_color/fps_depth_downsample_factor)
traj_save_path = "./data/human/{}/traj{}".format(date_str, episode_count)
...
data_save_path = "./data/human/{}/data_info.json".format(date_str)
